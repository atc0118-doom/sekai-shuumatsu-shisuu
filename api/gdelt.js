...
const safeFetch=async(url)=>{const r=await fetch(url,{headers:{'user-agent':'SAT-SYS-01/5.1'},cache:'no-store'}); if(!r.ok) throw new Error('upstream '+r.status); return r.json();};
function article(a){
  return {title:a.title||'No title',url:a.url||'',domain:a.domain||a.sourcecountry||'GDELT',source:a.sourcecountry||'',seen:a.seendate||''};
}
function scoreArticles(arts, keys){
  let mentions=0, severe=0, latest=[];
  for(const a of arts){
    const text=((a.title||'')+' '+(a.url||'')+' '+(a.seendate||'')).toLowerCase();
    if(keys.some(k=>text.includes(k))){
      mentions++;
      const hit=BAD.filter(w=>text.includes(w)).length;
      severe+=hit;
      if(latest.length<3) latest.push(article(a));
    }
  }
...
export default async function handler(req,res){
  ...
  try{
    ...
    const news=arts.slice(0,20).map(article).filter(n=>n.title && n.url);
    res.status(200).json({ok:true,source:'GDELT',updated:new Date().toISOString(),global,ranking,news});
  }catch(e){
    res.status(200).json({ok:false,error:String(e.message||e),updated:new Date().toISOString(),global:35,ranking:[],news:[]});
  }
}
