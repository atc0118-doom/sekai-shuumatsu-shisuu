...
{country:"ウクライナ",flag:"🇺🇦",score:78,reports:12,topic:"軍事衝突 / 重大",x:58.6,y:24.8,delta:8},
{country:"ロシア",flag:"🇷🇺",score:67,reports:10,topic:"軍事展開 / 警戒",x:60.5,y:20.2,delta:6},
{country:"イラン",flag:"🇮🇷",score:79,reports:9,topic:"中東緊張 / 重大",x:64.7,y:36.6,delta:4},
{country:"中国",flag:"🇨🇳",score:55,reports:7,topic:"軍事活動 / 警戒",x:78.9,y:34.5,delta:3},
{country:"台湾",flag:"🇹🇼",score:65,reports:6,topic:"台湾海峡 / 警戒",x:83.6,y:42.1,delta:3},
{country:"イスラエル",flag:"🇮🇱",score:47,reports:5,topic:"地域衝突 / 注意",x:59.7,y:37.2,delta:2},
{country:"アメリカ",flag:"🇺🇸",score:45,reports:4,topic:"軍事展開 / 観測",x:22.2,y:31.7,delta:1},
{country:"日本",flag:"🇯🇵",score:44,reports:4,topic:"周辺警戒 / 観測",x:88.3,y:33.8,delta:1}],
news:[
...
const ranks=data.ranking||fallback.ranking;renderRegions(ranks);$('rising').innerHTML=ranks.slice(0,3).map((r,i)=>`<div class="rise-card"><strong>${i+1}. ${r.flag} ${r.country}</strong><div class="delta">+${r.delta||i+4} ↑</div><p>${r.topic}</p><p>危険度：${r.score}</p></div>`).join('');
...
function sev(score){return score>=70?'high':score>=50?'mid':'low'}
function sevLabel(score){return score>=70?'重大':score>=50?'警戒':score>=30?'注意':'観測'}
function renderRegions(ranks){
  const box=$('regionCards');
  if(!box)return;
  box.innerHTML=ranks.slice(0,6).map((r,i)=>`<button class="region-card ${sev(r.score)}" data-i="${i}"><div class="region-top"><strong>${r.flag} ${r.country}</strong><span class="region-score">${r.score}</span></div><p class="region-topic">${sevLabel(r.score)} / ${r.topic}</p><div class="region-meta">危険報道 ${r.reports||0}件　変動 +${r.delta||0}</div></button>`).join('');
  box.querySelectorAll('.region-card').forEach(btn=>{
    btn.onclick=()=>{
      const r=ranks[Number(btn.dataset.i)];
      $('regionDetail').innerHTML=`<strong>${r.flag} ${r.country}</strong><br><span class="risk">危険度 ${r.score}</span> / ${level(r.score)}<br>${r.topic}<br>危険報道 ${r.reports||0}件　変動 +${r.delta||0}`;
    };
  });
}
